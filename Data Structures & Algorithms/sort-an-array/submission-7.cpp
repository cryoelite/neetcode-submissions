   ...
   public:
    ...
    vector<int>& selectionSort(vector<int>& nums) {
        ...
        for (size_t i{0}; i < nums.size(); ++i) {
            size_t min_index{
                i};  // we initialize the min index with the start elem of each iteration
    ...
    void merge(vector<int>& nums, int l1, const int l2, int r1, const int r2) {
        vector<int> temp{};
    ...
    vector<int>& mergeSort(vector<int>& nums, const int start, const int end) {
        ...
        // recursion, where say start is 4, end is 7, then mid is 3, now 3+1 is 4 which runs the
        // second branch again with 4,7. Not good. Another thing is, we don't need to static cast
        // here, because integers being divided use integer division
    ...
    vector<int> bucketSort(vector<int>& nums) {
        // The logic is simple, we store elements in buckets where each bucket stores a range of
        // values and the collection of buckets covers the entire input range. Then we sort the
        // buckets internally using stable sorting algorithms and finally we sequentially pull the
        // elements from the buckets and line them up in an output array.
    }
